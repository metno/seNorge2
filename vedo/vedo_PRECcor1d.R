...
path2input.1d.main<-paste(main.path.output,"/seNorge2/PRECcor1d",sep="")
...
path2input.1d.add<-paste(main.path.output,"/seNorge2_addInfo/PRECcor1d",sep="")
...
path2output.main<-paste(main.path.output,"/seNorge2_scratch/PRECcor1d",sep="")
path2output.main.grd<-paste(path2output.main,"/PREC",sep="")
path2output.add<-paste(main.path.output,"/seNorge2_scratch/PRECcor1d",sep="")
...
#------------------------------------------------------------------------------
# Plot analysis 
par.PRECcor1d<-list(col.scale=stepseq25.col,scale=scale.PREC1d,
                   main=paste(yyyy.mm.dd,"PRECcor1d","daily accumulated precipitation [UTC]"),
                   xlab="",ylab="",
...
aux<-PRECplot(namefileout=out.file.grd.ana,
              ...
              par=par.PRECcor1d)
# Plot IDI
scale.PRECcor1d.IDI<-seq(0,110,length=257)
scale.PRECcor1d.IDI[length(scale.PRECcor1d.IDI)]<-1000
#
r[]<-NA
# put data on raster variable (t=transpose)
r[]<-t(data.idi)
...
r[aux]<-rep(NA,length(aux))
par.PRECcor1d.IDI<-list(col.scale=banded.col,scale=scale.PRECcor1d.IDI,
                 main=paste(yyyy.mm.dd,"PRECcor1d/IDI","daily accumulated precipitation [UTC]",sep=" - "),
                 xlab="",ylab="",
                 xl=c(xlim.sw,xlim.ne),yl=c(ylim.sw,ylim.ne))
#
