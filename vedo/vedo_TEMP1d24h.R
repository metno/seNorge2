...
#ylim.ne<-7150000
#
scale.TEMP1d24h<-c(-1000,-35, -30,-25,-20,-15,-10,
                 -7.5, -5,-2.5,  0,  1,  3,  5,
...
# input directories
path2input.1d.main<-paste(main.path.output,"/seNorge2/TEMP1d24h",sep="")
...
path2input.1d.add<-paste(main.path.output,"/seNorge2_addInfo/TEMP1d24h",sep="")
...
in.1d.file.grd.idi<- paste(path2input.1d.add.grd,"/",yyyymm,
                           "/seNorge_v2_0_TEMP1d24h_grid_idi_",
                           yyyymmdd,".nc",sep="")
# output directories
dir.create(file.path(main.path.output,"seNorge2_scratch"), showWarnings = FALSE)
path2output.main<-paste(main.path.output,"/seNorge2_scratch/TEMP1d24h",sep="")
path2output.main.grd<-paste(path2output.main,"/TEMP",sep="")
path2output.add<-paste(main.path.output,"/seNorge2_scratch/TEMP1d24h",sep="")
...
#------------------------------------------------------------------------------
# Plot analysis 
par.TEMP1d24h<-list(col.scale=t2m_29lev.col,scale=scale.TEMP1d24h,
                 main=paste(yyyy.mm.dd,"TEMP1d24h","daily mean temperature [06-06 UTC]",sep=" - "),
                 xlab="",ylab="",
...
par.TEMP1h.IDI<-list(col.scale=banded.col,scale=scale.TEMP1h.IDI,
                 main=paste(yyyy.mm.dd,"TEMP1d24h/IDI","daily mean temperature [UTC]",sep=" - "),
                 xlab="",ylab="",
                 xl=c(xlim.sw,xlim.ne),yl=c(ylim.sw,ylim.ne))
#
