...
path2input.1d.add<-paste(main.path.output,"/seNorge2_addInfo/PREC1hRT",sep="")
...
path2output.main<-paste(main.path.output,"/seNorge2_scratch/PREC1hRT",sep="")
path2output.main.grd<-paste(path2output.main,"/PREC",sep="")
path2output.add<-paste(main.path.output,"/seNorge2_scratch/PREC1hRT",sep="")
...
aux<-PRECplot(namefileout=out.file.grd.ana,
              ...
              par=par.PREC1hRT)
# Plot IDI
scale.PREC1hRT.IDI<-seq(0,110,length=257)
scale.PREC1hRT.IDI[length(scale.PREC1hRT.IDI)]<-1000
#
r[]<-NA
# put data on raster variable (t=transpose)
r[]<-t(data.idi)
...
r[aux]<-rep(NA,length(aux))
par.PREC1hRT.IDI<-list(col.scale=banded.col,scale=scale.PREC1hRT.IDI,
                 main=paste(yyyy.mm.dd.hh,"PREC1hRT/IDI","hourly accumulated precipitation [UTC]",sep=" - "),
                 xlab="",ylab="",
                 xl=c(xlim.sw,xlim.ne),yl=c(ylim.sw,ylim.ne))
#
