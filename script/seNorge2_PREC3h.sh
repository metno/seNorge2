...
# COMMAND LINE:
# =============
#  >seNorge2_PREC3h.sh -s yyyy.mm.dd  (date start)
...
#  BLACKL=$MAINDIR/etc/blacklists/seNorge2_PREC3h_blacklist.txt
#  ERROBS=$MAINDIR/etc/suspect_observations/seNorge2_PREC3h_suspect_observations.txt
#--------------------------------------------------
#  Clean temporary directories, if needed 
#--------------------------------------------------
#  echo "seNorge2_PREC3h.sh "`date +%Y-%m-%d" "%H:%M`" > clean directories"
...
# Exit 
#--------------------------
  echo "seNorge2_PREC3h.sh "`date +%Y-%m-%d" "%H:%M`" > success!"
  exit 0
