...
# COMMAND LINE:
# =============
#  >seNorge2_PREC1d.sh -s yyyy.mm.dd  (date start)
...
#------------------------------------------------------------------------------
# log
  echo "seNorge2_PREC1d.sh "`date +%Y-%m-%d" "%H:%M`" > elaborations from "$DATESTART" UTC to "$DATEEND" UTC"
...
# Exit 
#--------------------------
  echo "seNorge2_PREC1d.sh "`date +%Y-%m-%d" "%H:%M`" > success!"
  exit 0
